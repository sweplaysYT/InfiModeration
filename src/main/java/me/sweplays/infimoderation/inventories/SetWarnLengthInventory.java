...
public class SetWarnLengthInventory {
    ...
    private static final Date warnLength = new Date();

    public static void initItems(OfflinePlayer target) {

        GuiItem lengthOne = new GuiItem(createItem(Material.PAPER, 1, "§c7d"), event -> {
            getWarnLength().setTime(System.currentTimeMillis() + 7 * 24 * 60 * 60 * 1000);
            event.getWhoClicked().closeInventory();
            ConversationFactory conversationFactory = new ConversationFactory(plugin);
            Conversation conversation = conversationFactory.withFirstPrompt(new WarnReasonConversation(target)).withLocalEcho(false).buildConversation((Conversable) event.getWhoClicked());
            conversation.begin();
            permanent = false;
            lengthString = "7d";
        ...
        GuiItem lengthTwo = new GuiItem(createItem(Material.PAPER, 1, "§c15d"), event -> {
            getWarnLength().setTime(System.currentTimeMillis() + 15 * 24 * 60 * 60 * 1000);
            event.getWhoClicked().closeInventory();
            ConversationFactory conversationFactory = new ConversationFactory(plugin);
            Conversation conversation = conversationFactory.withFirstPrompt(new WarnReasonConversation(target)).withLocalEcho(false).buildConversation((Conversable) event.getWhoClicked());
            conversation.begin();
        ...
        GuiItem lengthThree = new GuiItem(createItem(Material.PAPER, 1, "§c30d"), event -> {
            getWarnLength().setTime(System.currentTimeMillis() + 30L * 24 * 60 * 60 * 1000);
            event.getWhoClicked().closeInventory();
            ConversationFactory conversationFactory = new ConversationFactory(plugin);
            Conversation conversation = conversationFactory.withFirstPrompt(new WarnReasonConversation(target)).withLocalEcho(false).buildConversation((Conversable) event.getWhoClicked());
            conversation.begin();
        ...
        GuiItem lengthFour = new GuiItem(createItem(Material.PAPER, 1, "§c2mo"), event -> {
            getWarnLength().setTime(System.currentTimeMillis() + 60L * 24 * 60 * 60 * 1000);
            event.getWhoClicked().closeInventory();
            ConversationFactory conversationFactory = new ConversationFactory(plugin);
            Conversation conversation = conversationFactory.withFirstPrompt(new WarnReasonConversation(target)).withLocalEcho(false).buildConversation((Conversable) event.getWhoClicked());
            conversation.begin();
        ...
        GuiItem lengthFive = new GuiItem(createItem(Material.PAPER, 1, "§c4mo"), event -> {
            getWarnLength().setTime(System.currentTimeMillis() + 120L * 24 * 60 * 60 * 1000);
            event.getWhoClicked().closeInventory();
            ConversationFactory conversationFactory = new ConversationFactory(plugin);
            Conversation conversation = conversationFactory.withFirstPrompt(new WarnReasonConversation(target)).withLocalEcho(false).buildConversation((Conversable) event.getWhoClicked());
            conversation.begin();
        ...
        GuiItem lengthSix = new GuiItem(createItem(Material.PAPER, 1, "§c1y"), event -> {
            getWarnLength().setTime(System.currentTimeMillis() + 360L * 24 * 60 * 60 * 1000);
            event.getWhoClicked().closeInventory();
            ConversationFactory conversationFactory = new ConversationFactory(plugin);
            Conversation conversation = conversationFactory.withFirstPrompt(new WarnReasonConversation(target)).withLocalEcho(false).buildConversation((Conversable) event.getWhoClicked());
            conversation.begin();
    ...
    public static Date getWarnLength() {
        return warnLength;
    }
